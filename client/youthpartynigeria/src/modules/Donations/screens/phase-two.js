...
const DonationPhaseTwoComponent = ({  data }) => (
  <View style={{ flex: 1, justifyContent: 'space-around', backgroundColor: '#F4F6F6', paddingLeft: width * 0.11 }}>
    { /* The Amount */}
    <View style={{ height: height * 0.12, width }}>
      ...
      <View style={{ minHeight: height * 0.09, width, flexDirection: 'row', flexWrap: 'nowrap', paddingLeft: width * 0.02 }}>
        <View style={{ height: height * 0.06, width: width * 0.12, backgroundColor: defaultGreen, justifyContent: 'center', paddingRight: 3,  marginBottom: 25, alignItems: 'center', borderLeftRadius: 5, }}> <Text style={{ fontSize: 19, textAlign: 'center',fontWeight: '700', color: 'white'}}> N </Text></View>
        <TextInput
          style={{ height: height * 0.06, width: width * 0.65,  backgroundColor: 'white', color: '#B3B6B7', fontSize: 13, marginLeft: -5, fontWeight: '500', paddingLeft: 20, borderWidth: 0.65, borderColor: '#38393950', borderRadius: 2 }}
          />
      </View>
  </View>
   { /* Render the target */}
...
const Target = ({ data }) => (
  <View style={{ height: height * 0.18, width, flexDirection: 'row', flexWrap: 'nowrap', }}>
    <Image  style={{ height: 40, width: 40, borderRadius: 20, marginRight: 10}} source={{ uri: data.avatar }}/>
    <View style={{ height: height * 0.15, maxWidth: width * 0.5}}>
      <Text style={{ fontSize: 14, fontWeight: '600', color: '#373838', marginBottom: 5 }}> { data.name } </Text>
